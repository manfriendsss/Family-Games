    ...
    <motion.div initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
      <section className="bg-[#1D1D1F] rounded-3xl p-6 shadow-xl text-white">
        ...
        <ul className="space-y-2 text-xs font-bold text-gray-300 leading-relaxed">
          ...
          <li>- Mỗi đội có 120 giây để đoán càng nhiều từ càng tốt.</li>
    ...
    return (
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-4">
        <section className="bg-white rounded-3xl p-6 shadow-sm border border-gray-100 text-center">
          <h3 className="text-2xl font-black text-gray-900 mb-2">Đội {team.name} hết thời gian</h3>
          ...
          <button onClick={nextTeam} className="w-full h-12 mt-4 rounded-xl bg-gray-900 text-white font-black active:scale-95 transition-transform">
            {orderPos >= playOrder.length - 1 ? 'XEM B\u1ea2NG \u0110I\u1ec2M CU\u1ed0I' : `ĐẼN ĐỘI ${playOrder[orderPos + 1].name}`}
          </button>
        </section>
      </motion.div>
    );
    ...
    return (
      <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="space-y-4">
        <section className="bg-white rounded-3xl p-5 shadow-sm border border-gray-100">
          <p className="text-xs font-black text-gray-400 uppercase tracking-widest mb-2">Phân đội</p>
          <div className="space-y-2">
            {teams.map((t) => (
              <div key={t.name} className="rounded-xl border border-gray-100 p-3">
                <p className="text-sm font-black text-gray-800">Đội {t.name}</p>
        ...
        <section className="bg-white rounded-3xl p-5 shadow-sm border border-gray-100 text-center">
          <p className="text-sm font-black text-gray-600">Đội bắt đầu: <span className="text-blue-600">Đội {team.name}</span></p>
          <p className="text-xs font-bold text-gray-500 mt-1">Đội {team.name} bấm sẵn sàng để bắt đầu lượt.</p>
          <button
            onClick={() => {
              setTimeLeft(TURN_SECONDS);
              goNextCard();
      ...
      <section className="bg-white rounded-3xl p-4 shadow-sm border border-gray-100">
        <div className="flex items-center justify-between">
          ...
          <div className="text-right">
            <p className="text-xs font-black text-gray-400 uppercase tracking-widest">Thời gian</p>
        ...
        <p className="text-xs font-bold text-gray-500">Đội khác cầm máy để chấm và giữ cho người mô tả xem.</p>
      </section>
